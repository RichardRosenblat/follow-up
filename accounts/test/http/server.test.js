...
describe("Follow-up Server", () => {
	describe("Health check", () => {
		...
		it("health check should return ok and connected if database is not connected", async () => {
			disableMock();
			const response = await request(app).get("/").expect(200);
	...
	describe("POST /users", () => {
		...
		it("Should return an array with errors when name is empty", async () => {
			const response = await request(app)
				...
				.expect(400);
			expect(response.body).toStrictEqual([{ field: "name", message: "Name cannot be empty" }]);
		});
		it("Should return an array with errors when email is not valid", async () => {
			const response = await request(app)
				...
				.expect(400);
			expect(response.body).toStrictEqual([{ field: "email", message: "Email must be a valid email adress" }]);
		});
		it("Should return an array with errors when email repeated", async () => {
			...
			const response = await request(app)
				...
				.expect(400);
			expect(response.body).toStrictEqual([{ field: "email", message: "Email must be unique" }]);
		});
