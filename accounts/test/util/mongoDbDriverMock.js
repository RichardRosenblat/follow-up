...
export function mockMongoDriver(collection) {
    return {
        ...
        findOne: ({ _id, email }) => {
            return getCollectionMock(collection).find(
                ({ _id: mongoDocId, email: mongoDocEmail }) => {
                    return _id
                        ? mongoDocId.toHexString() === _id.toHexString()
                        : email === mongoDocEmail;
                }
            );
        },
        insertOne: (obj) => {
            getCollectionMock(collection).push(obj);
        },
        updateOne: ({ _id }, { $set, $push, $pull }) => {
            ...
            if ($set) {
                collectionToUpdate[foundDocIndex] = {
                    ...collectionToUpdate[foundDocIndex],
                    ...$set,
                };
                return true;
...
function getCollectionMock() {
    if (isDisabled) {
        throw new Error("Mock disconnected error");
    }
    return userCache;
}
